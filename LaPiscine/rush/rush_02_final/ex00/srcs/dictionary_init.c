...
/*   Updated: 2022/01/25 10:54:31 by youngcho         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// init load하는 과정이 너무 어렵네요... 이해가 안갑니다..
...
void	dict_init_insert_temp(t_dict_init *this, char *buf, int count)
{
	...
	// tail에 chain을 주는 이유가 궁금합니다.
	// 다음 read()에서 들어오는 buf를 tail의 next에 넣어주기 위함인가요?
...
void	dict_init_insert_final(t_dict_init *this, unsigned int length)
{
	...
	// final_tail에 chain 넣는 이유.....?
	// (final_tail->next에 chain넣고, final_tail에 chain을 넣는다?)
	this->final_tail = chain;
	// sizeof(*chain->str)의 의미가 뭔가요?
